...
public class PartitionTest extends TestCase {
	...
	public void testGMLPartition() throws IOException {
		...
		try {
			...
			SliceManager sliceManager = new SliceManager(partitionUUID, new JavaSliceSerializer(), new FileStorageManager(sliceDir));
			sliceManager.writePartition(gmlPartition);
	
			// Read the partition
			sliceManager = new SliceManager(partitionUUID, new JavaSliceSerializer(), new FileStorageManager(sliceDir));
