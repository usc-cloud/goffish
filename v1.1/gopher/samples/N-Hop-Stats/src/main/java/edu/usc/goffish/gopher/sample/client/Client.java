...
public class Client {
    ...
    public static void main(String[] args) throws ClassNotFoundException, InstantiationException, IllegalAccessException {
        ...
        if(args.length != 5) {
            throw new RuntimeException("args[0]=Host to connect + arg[1]=data port arg[2]=control port arg[3] = N  , arg[4] = list");
        }
        ...
        Message<byte[]> data = MessageFactory.createDefaultMessage();

        BSPMessage dataMsg  = new BSPMessage();

        String msgS = N + "#" + args[4];

        dataMsg.setData(msgS.getBytes());
        dataMsg.setType(BSPMessage.DATA);
        data.putPayload(BitConverter.getBytes(dataMsg));

        sender.send(data);

        Message<byte[]> ctrl = MessageFactory.createDefaultMessage();
