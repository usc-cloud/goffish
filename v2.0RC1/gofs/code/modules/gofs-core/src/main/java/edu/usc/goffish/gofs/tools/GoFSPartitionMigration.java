...
public class GoFSPartitionMigration {
	...
	public static void main(String[] args) throws IOException {
		...
		try {
			config.load(Files.newInputStream(partitionFilePath.resolve(GOFS_ROOT_DIR).resolve(DataNode.DATANODE_CONFIG)));
			...
			migrationGoFSRootUri = new URI("file", remoteHost != null ? remoteHost : "", new StringBuffer(migrationFileUri.getPath())
			.append(java.io.File.separatorChar).append("gofs").append(java.io.File.separatorChar).toString(), null);
			...
			SCPHelper.SCP(partitionFilePath.resolve(GOFS_ROOT_DIR).resolve(DataNode.DATANODE_SLICE_DIR), UriBuilder.fromUri(migrationFileUri).path("gofs").build());
		} catch (Exception e) {
