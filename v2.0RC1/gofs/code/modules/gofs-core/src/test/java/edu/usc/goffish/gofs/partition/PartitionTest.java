...
public class PartitionTest extends TestCase {
	...
	public void testGMLPartition() throws IOException {
		...
		try {
			...
			ISliceManager sliceManager = SliceManager.create(partitionUUID, new JavaSliceSerializer(), new FileStorageManager(sliceDir));
			sliceManager.writePartition(gmlPartition, 1, gmlPartition.size());

			// Read the partition
			sliceManager = SliceManager.create(partitionUUID, new JavaSliceSerializer(), new FileStorageManager(sliceDir));
	...
	protected void comparePartitionInstances(IPartition actual) throws IOException {
		ISubgraph subgraph = actual.getSubgraph(8L);
