...
public class PageRankSubgraph extends GopherSubGraph {
	...
	private Iterator<? extends ISubgraphInstance> _instanceIterator;
	private ISubgraphInstance _currentInstance;
	...
	private void initialize() throws IOException {
		...
		_instanceIterator = subgraph.getInstances(Long.MIN_VALUE, Long.MAX_VALUE, subgraph.getVertexProperties().subset(Collections.singleton("is_exists")), subgraph.getVertexProperties().subset(Collections.singleton("is_exists")), false).iterator();
		_currentIteration = -1;
	}
	...
	public void compute(List<SubGraphMessage> stuff) {
		...
		if (_currentIteration != getIteration()) {
			_currentIteration = getIteration();
			if (_instanceIterator.hasNext()) {
				_currentInstance = _instanceIterator.next();
			} else {
		...
		if (superStep == 1) {
			...
			for (ITemplateVertex vertex : subgraph.vertices()) {
				if (!vertex.isRemote() && ((Boolean)_currentInstance.getPropertiesForVertex(vertex.getId()).getValue("is_exists")).booleanValue()) {
	...
	private void outputToFile() {
		// output results
		try(BufferedWriter output = new BufferedWriter(new OutputStreamWriter(Files.newOutputStream(Paths.get("it_" + getIteration() + "_ss_" + superStep + "_subgraph_" + subgraph.getId() + "_output"))))) {
			for (ITemplateVertex vertex : subgraph.vertices()) {
				if (!vertex.isRemote()) {
	...
	private double doPageRankIteration(Iterable<Message> messages, boolean updateValues, boolean sendMessages) {
		...
		for (ITemplateVertex vertex : subgraph.vertices()) {
			...
			for (ITemplateEdge edge : vertex.outEdges()) {
				if (!((Boolean)_currentInstance.getPropertiesForEdge(edge.getId()).getValue("is_exists")).booleanValue()) {
					continue;
				}
